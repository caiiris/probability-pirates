 ...
 */
export function newMilestonesFor(streak: number, alreadyReached: string[] | undefined): MilestoneId[] {
  const reached = alreadyReached ?? [];
  return MILESTONE_THRESHOLDS
    .filter((t) => streak >= t && !reached.includes(`streak-${t}`))
    .map((t) => `streak-${t}` as MilestoneId);
}
