...
### D117 — Captain's Wager: app-wide estimation game to train calibration (the third learning loop)

- **Status:** resolved (2026-06-27, design locked, unbuilt) — see [`spec-captains-wager`](specs/spec-captains-wager.md) for the full spec
- **Chose:** A standalone destination at `/wager`: one probability-flavored Fermi-style prompt every **3 days**, same prompt for everyone, **single numeric guess**, **reveal-on-submit** (no global drop moment), **log-distance scoring 0–100**, **distribution-style feedback** (histogram of all guesses + "you beat X% of wagerers") instead of a ranked leaderboard, and a mandatory **2-3 sentence Captain Pascal teach-back** that names the probability concept and (where applicable) links to the related course-path lesson. Submission window is **open forever** (a learner who arrives late can still submit to wager #1). Wager attempts are **not** a learner-model signal — they live in a separate `wagerStats` summary, distinct from `model.mastery` and Elo (D-CW8 in the spec). Launch with **30 hand-authored prompts** (~90 days runway), AI-generation with human review layered in Phase 2.
- **Why a third loop?** The course path teaches *technique*; practice drills *fluency*; neither trains **calibration** — the metacognitive skill of putting a number on how confident you are and learning from being wrong. Calibration is research-validated as trainable (Greenberg et al., 2024 RCT: <30min training reduced overconfidence; Tetlock's *Good Judgment Project* showed multi-year persistence). And the live estimation-game market (Estimania, Napkin, HowMany, Magnitudle, Fermi Questions, defunct Brilliant Daily) shipped *zero* with a teach-back — that's the empty niche.
- **Considered:**
  - **Daily cadence** (Wordle/Estimania pattern). Rejected — daily content is unsustainable hand-authored; our audience's primary loop is lessons, not Wager; 3-day cadence lets the histogram fill before each viewer.
  - **Hard 72-hour window + global reveal moment.** Most habit-forming but punishes latecomers and adds a second streak to manage (D-CW4).
  - **Ranked global leaderboard.** Standard in the field; rejected because it motivates the top 5% and deflates everyone else (Dweck performance-vs-mastery framing). Distribution-style "you beat X%" is positional but not ranked, and motivates the whole user base (D-CW3).
  - **Confidence-interval input** (low/high bound), the pedagogically purest calibration form. Deferred to Phase W4 — LessWrong A/B (Calibrate, N=11) found that CI calibration trains *less* effectively than point-estimate calibration for novices, and CI input is higher-friction on mobile (D-CW2).
  - **AI-generate content from day one.** Rejected — LLM-generated probability claims need source verification, and the teach-back is the differentiator (D-CW1); outsourcing the core artifact weakens the product. AI is a Phase-W2 candidate-generator with human curation (D-CW5).
  - **Use Wager attempts as a learner-model signal.** Rejected — wager prompts are *far* from practice templates (real-world Fermi vs. textbook setups), too sparse (~10/month) to meaningfully update mastery, and would distort the model. Calibration is tracked separately as `wagerStats.averageLogError` (D-CW8).
  - **Home banner / Home card.** Rejected per user direction — competes with the path's primacy. Wager gets its own sidebar item with an unread dot (D-CW7).
- **Gaps / risks:**
  - **Cold start.** Below N=20 submissions the histogram is statistical noise. Mitigation: suppress histogram + "beat X%" callout below N=20, show answer + teach-back only. Do *not* seed synthetic guesses (would corrupt the calibration instrument). Subtle "previous frequency wager had this shape" overlay layered in W2 (D-CW9).
  - **Content authoring bandwidth.** 30 hand-authored prompts is real work — each needs a sourced answer + a teach-back that names the concept. Mitigation: tracked as a Phase-W1 launch blocker; AI candidate generation in W2 unblocks the cadence.
  - **Client-authoritative scoring** matches the rest of the app (D-AS1 et al.) but means a determined user can fake a `score: 100`. Accepted for MVP because there's no reward gating; promoted to server-side scoring only if Wager outcomes ever drive real rewards (W3 trigger).
  - **Histogram fetch cost** at scale: client-side bin of `/wagers/{id}/submissions` works to ~1000 docs/wager; popular wagers need pre-aggregation via Cloud Function in W3.
- **See also:** [`spec-captains-wager`](specs/spec-captains-wager.md) (full design), D27 (in-app reminders only — Wager doesn't push), D42 (XP progression — Wager XP is small and separate to avoid competing with practice/lessons), D107/D111/D116 (mastery-vs-exposure metric discipline; Wager's `averageLogError` follows the same "real signal, not vanity" rule).

### D118 — Captain's Wager: two-step submit (placeholder create → score-patch) to reconcile gated answer with client-side scoring

- **Status:** resolved (2026-06-27, implemented in WP-CW-D/WP-CW-F fix) — see [`spec-captains-wager` D-CW11](specs/spec-captains-wager.md) and [`wp-captains-wager` R-W8](specs/wp/wp-captains-wager.md)
- **Problem:** `submitWager` originally required `trueAnswer: number` from the caller so it could compute the log-distance score before writing the submission. But `/wagers/{id}/private/answer` is gated behind Firestore rules that deny reads until the user's own submission EXISTS (the `exists()` no-peeking gate). The pre-submit caller cannot read `trueAnswer` — it's a circular dependency. Every submission was shipping `score: 0` as a permanent placeholder.
- **Chose:** A **two-step flow** inside `submitWager`: (1) write a placeholder submission `{ score: 0, logError: 0 }` via `setDoc` — this commits the user's guess and satisfies the `exists()` gate; (2) read the (now-unlocked) answer doc; (3) compute the real score; (4) patch the submission + update stats + grant XP in a single transaction. A **relaxed update rule** (C-W4.5) on the submission subcollection allows this one-time score-patch only from placeholder state (both `score == 0 && logError == 0`). Once either field is non-zero the gate closes permanently, preserving the guess-immutability invariant. A **`ensureSubmissionScored` self-healer** retries the score-patch if the step-4 transaction fails (network), triggered by `useUserSubmission` detecting placeholder state via a `useRef`-guarded effect.
- **Considered:**
  - **Public `trueAnswer` on the wager doc.** Rejected — defeats the no-peeking property; users could read the answer before submitting by querying the field directly.
  - **Cloud Function that writes score server-side.** Rejected — Cloud Functions are out of scope for Phase 1 and would be overkill for a client-authoritative scoring system (D-CW8 / R-W3).
  - **Pass `trueAnswer` from the caller.** Rejected — circular (caller can't read the gated doc before submitting; this was the original bug).
  - **Remove the no-peeking rule entirely.** Rejected — the no-peeking gate is load-bearing for the calibration instrument: the whole point is that the user commits a number *before* seeing the true answer. Removing the gate lets anyone read the answer without submitting, breaking the pedagogical core.
- **Trade-offs accepted:** two Firestore round-trips per submit (vs. one with a Cloud Function). The self-healing retry on network failure makes the extra round-trip robust. Stats are only updated after the real score is known (the placeholder is transient and must not pollute the averages).
- **See also:** [`spec-captains-wager` D-CW11](specs/spec-captains-wager.md) (full decision text), R-W3 (client-authoritative scoring), R-W8 (C-W5 contract change), D117 (the wager feature itself).
